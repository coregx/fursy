#!/usr/bin/env bash
# Pre-Release Validation Script for FURSY HTTP Router
...
echo ""
echo "================================================"
echo "  FURSY HTTP Router - Pre-Release Check"
echo "================================================"
echo ""
...
if [ -d "internal" ]; then
    ...
    if [ -n "$COVERAGE" ]; then
        ...
        if awk -v cov="$COVERAGE" 'BEGIN {exit !(cov >= 85.0)}'; then
            log_success "Coverage meets Phase 1 requirement (>85%)"
        else
            log_warning "Coverage below 85% (${COVERAGE}%) - Phase 1 target"
            WARNINGS=$((WARNINGS + 1))
        fi
    else
...
if [ -n "$OVERALL_COVERAGE" ]; then
    ...
    if awk -v cov="$OVERALL_COVERAGE" 'BEGIN {exit !(cov >= 85.0)}'; then
        log_success "Overall coverage meets requirement (>85%)"
    else
        log_warning "Overall coverage below 85% (${OVERALL_COVERAGE}%)"
        WARNINGS=$((WARNINGS + 1))
    fi
else
...
if [ -f "go.mod" ]; then
    ...
    # Expected dependencies: golang-jwt/jwt (JWT) and golang.org/x/time (RateLimit)
    EXPECTED_DEPS=2

    if [ "$CORE_DEPS" -eq 0 ]; then
        log_success "Core has zero external dependencies ✅"
    elif [ "$CORE_DEPS" -eq "$EXPECTED_DEPS" ]; then
        log_success "Minimal dependencies policy met (JWT + RateLimit) ✅"
        log_info "  • JWT authentication: golang-jwt/jwt"
        log_info "  • Rate limiting: golang.org/x/time"
    else
        log_warning "Unexpected number of dependencies: $CORE_DEPS (expected: 0 or $EXPECTED_DEPS)"
