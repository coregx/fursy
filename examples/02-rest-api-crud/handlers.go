...
func (h *Handlers) CreateUser(c *fursy.Box[CreateUserRequest, UserResponse]) error {
	...
	if c.ReqBody.Email == "" {
		return c.Problem(fursy.BadRequest("Email is required"))
	}
	if c.ReqBody.Username == "" {
		return c.Problem(fursy.BadRequest("Username is required"))
	}
	if c.ReqBody.FullName == "" {
		return c.Problem(fursy.BadRequest("Full name is required"))
	}
	if c.ReqBody.Age < 18 || c.ReqBody.Age > 120 {
		return c.Problem(fursy.BadRequest("Age must be between 18 and 120"))
	}
...
func (h *Handlers) DeleteUser(c *fursy.Box[fursy.Empty, fursy.Empty]) error {
	// Get ID from URL parameter.
	idStr := c.Param("id")
	id, err := strconv.Atoi(idStr)
	if err != nil {
